...
#all these are lists containing certain key words that may be said by user
said = ['hi','hello']
question = ['what time is it','what\'s the time',]
meaning = ['meaning']
ctime = time.asctime( time.localtime(time.time()))#displays current time to user
quit = ['stop','exit','quit']
trans = ['translate']
...
def rec():
     while True: #loops the code block below until the user says 'stop' or 'quit'
            ...
            with sr.Microphone() as source: #opens the microphone 
                ...
                audio = r.listen(source) #listens audio from mic(Calls the listen function)
